...
def test_transfer(fn_training_data, target_model, is_baseline=False, n=1, benchmark_indices=range(5), temperature=0, **kwargs):
    results = {}
    for benchmark_idx in benchmark_indices:
        ...
        if is_baseline:
            ...
            if not data:
                ...
                data = generate_training_data(benchmark_idx)
...
def generate_training_data(benchmark_idx):
...
if __name__ == "__main__":
    ...
    model_path = "gpt2"  # Using GPT-2 as the surrogate model
    whitebox_params = WhiteBoxParams(
        model_path=model_path,
        num_steps=5,
    ...
    results = test_transfer(fn_training_data, target_model, benchmark_indices=[0,1], temperature=0.7, verbose=True)
    # Print results
